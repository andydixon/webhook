...
var hostName = func() string {
	...
	return "webhook.me.uk"
}()

// emailShell wraps a body of sections in the shared design: stylesheet, header, footer.
...
<style>
  body { margin:0; padding:0; background:#f4f6fa; color:#0a0a0a; -webkit-font-smoothing:antialiased; font-family:{{.Sans}}; font-size:15px; line-height:1.6; }
  a { color:#0047ab; }
  table { border-collapse:separate; border-spacing:0; }
  .frame { width:100%; max-width:660px; }
  .hero { background:#ffffff; padding:30px 34px 22px; border-radius:12px 12px 0 0; border:1px solid #e3e7ef; border-bottom:0; border-top:4px solid #0047ab; }
  .kicker { font-family:{{.Mono}}; font-size:11px; letter-spacing:.18em; text-transform:uppercase; color:#0047ab; margin:0 0 10px; }
  .hero h1 { margin:0 0 12px; font-size:24px; font-weight:800; letter-spacing:-.02em; line-height:1.25; color:#0a0a0a; }
  .stamp { display:inline-block; font-family:{{.Mono}}; font-size:12px; letter-spacing:.04em; color:#4a4f5c; }
  .card { background:#ffffff; padding:0 0 26px; border-radius:0 0 12px 12px; border:1px solid #e3e7ef; border-top:0; }
  .section { padding:22px 34px 0; }
  .section-title { font-family:{{.Mono}}; font-size:11px; font-weight:700; letter-spacing:.16em; text-transform:uppercase; color:#0047ab; margin:0 0 10px; line-height:1.4; }
  .subsection { margin:14px 0 0; }
  .subsection-title { font-size:13px; font-weight:700; color:#0a0a0a; margin:16px 0 8px; }
  table.metadata { width:100%; border:1px solid #e3e7ef; border-radius:8px; overflow:hidden; background:#ffffff; }
  td.metadata-label, td.metadata-value { padding:10px 14px; border-bottom:1px solid #eef0f5; vertical-align:top; text-align:left; }
  tr:last-child > td.metadata-label, tr:last-child > td.metadata-value { border-bottom:0; }
  td.metadata-label { width:150px; max-width:170px; font-family:{{.Mono}}; font-size:12px; line-height:1.5; color:#4a4f5c; background:#f6f8fc; word-break:break-word; }
  td.metadata-value { font-size:14px; color:#0a0a0a; word-break:break-word; }
  td.metadata-value table.metadata { margin:2px 0; }
  .data-box { margin:0; padding:16px 18px; background:#0a0a0a; color:#e8e8e8; border-radius:8px; font-family:{{.Mono}}; font-size:12.5px; line-height:1.65; white-space:pre-wrap; word-break:break-word; overflow-x:auto; }
  .path-title { display:inline-block; margin:18px 0 8px; font-family:{{.Mono}}; font-size:12px; font-weight:700; color:#0047ab; }
  .array-item { margin:0 0 12px; padding:16px; background:#f6f8fc; border:1px solid #e3e7ef; border-radius:8px; }
  .array-item-title { font-size:14px; font-weight:800; margin:0 0 10px; color:#0a0a0a; letter-spacing:-.01em; }
  .array-item-simple { margin:0 0 6px; padding:9px 14px; background:#f6f8fc; border:1px solid #e3e7ef; border-radius:8px; font-size:14px; }
  .simple-value { font-size:14px; padding:4px 0; }
  .commit-item { margin:0 0 10px; padding:12px 14px; background:#f6f8fc; border:1px solid #e3e7ef; border-radius:8px; }
  .commit-item .sha { display:inline-block; font-family:{{.Mono}}; font-size:12px; font-weight:700; color:#0047ab; text-decoration:none; }
  .commit-item .who { font-size:12px; color:#4a4f5c; margin-left:6px; }
  .commit-item .msg { margin-top:6px; font-size:14px; line-height:1.5; color:#0a0a0a; }
  .message-box { margin:12px 0; padding:14px 18px; background:#eef3fb; border-left:3px solid #0047ab; border-radius:0 8px 8px 0; font-size:16px; line-height:1.55; color:#0a0a0a; }
  .error-box { margin:12px 0; padding:12px 14px; background:#fef2f2; border-left:3px solid #dc2626; border-radius:0 8px 8px 0; }
  .footer { padding:20px 34px 8px; text-align:center; color:#7a8090; font-family:{{.Mono}}; font-size:11px; letter-spacing:.06em; line-height:1.8; }
  .footer a { color:#0047ab; text-decoration:none; }
  .footer .bolt { color:#0047ab; }
</style>
</head>
